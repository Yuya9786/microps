...
tcp_retransmit_queue_emit(void *arg, void *data)
{
    ...
    timeout = entry->last;
    timeval_add_usec(&timeout, entry->rto); /* caluculate retransmit time */
    if (timercmp(&now, &timeout, >)) { /* if the retransmit time has passed */
...
tcp_output(struct tcp_pcb *pcb, uint8_t flg, uint8_t *data, size_t len)
{
    ...
    if (TCP_FLG_ISSET(flg, TCP_FLG_SYN | TCP_FLG_FIN) || len) { /* store only segments that consume seq number into retransmit queue. (not ack, rst) */
...
tcp_segment_arrives(struct tcp_segment_info *seg, uint8_t flags, uint8_t *data, size_t len, struct tcp_endpoint *local, struct tcp_endpoint *foreign)
{
    ...
    case TCP_PCB_STATE_LISTEN:
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            /* but processing of SYN and ACK should not be repoeated */
            return;
        }
    ...
    case TCP_PCB_STATE_SYN_SENT:
         ...
         */
        if (TCP_FLG_ISSET(flags, TCP_FLG_ACK)) {
            if (seg->ack <= pcb->iss || seg->ack > pcb->snd.nxt) { /* if the ack is for unsent seq, send rst */
                tcp_output_segment(seg->ack, 0, TCP_FLG_RST, 0, NULL, 0, local, foreign);
                return;
            }
        ...
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            pcb->rcv.nxt = seg->seq + 1; 
            pcb->irs = seg->seq;
            if (acceptable) { /* if accepted ack */
                pcb->snd.una = seg->ack; /* update unconfirmed seq (ack show "next value to recv") */
    ...
    case TCP_PCB_STATE_SYN_RECEIVED:
        ...
        /* fall through */
    case TCP_PCB_STATE_ESTABLISHED:
        if (pcb->snd.una < seg->ack && seg->ack <= pcb->snd.nxt) { /* receive ack include unreceived seq */
            pcb->snd.una = seg->ack;
            tcp_retransmit_queue_cleanup(pcb);
...
tcp_open_rfc793(struct tcp_endpoint *local, struct tcp_endpoint *foreign, int active)
{
    ...
    if (active) {
        ...
        pcb->foreign = *foreign;
        pcb->rcv.wnd = sizeof(pcb->buf);
        pcb->iss = random(); /* number the initial value of seq */
        ...
        pcb->snd.una = pcb->iss; /* set iss as unconfirmed seq */
        pcb->snd.nxt = pcb->iss + 1; /* set next seq */
